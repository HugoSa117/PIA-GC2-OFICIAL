    ...
    case WM_MOUSEMOVE: {
        ...
        if (gamePad->IsConnected())
        {
            ...
            if (velocidad > 0.19 || velocidad < -0.19) {
                ...
                if (velocidad > 0.19) dxrr->vel = velocidad * 2.5;
                else if (velocidad < -0.19) dxrr->vel = velocidad * 2.5;
            }

            float velocidad2 = (float)gamePad->GetState().Gamepad.sThumbLX / 32767.0;
            if (velocidad2 > 0.19 || velocidad2 < -0.19) {
                if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
                    velocidad2 *= 14.5;
                else if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) velocidad2 /= 3;
                else velocidad2 *= 2.5;
